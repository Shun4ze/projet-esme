import os  # Ajoute cette ligne en haut de ton script
from pyspark.sql import SparkSession
from pyspark.sql.types import StructType, StructField, StringType, DoubleType, LongType, ArrayType
from pyspark.sql.functions import (
    col, from_json, from_unixtime, to_timestamp, expr, to_json, struct, when, unix_timestamp
)
...
# Calcul de la colonne "severity_index"
processed_df = processed_df.withColumn(
    "severity_index",
    (col("speed") * 0.5) + ((1015 - col("pressure")) * 0.3) + (col("humidity") * 0.2)
)

# Calcul de la colonne "time_of_day" basée sur l'heure
processed_df = processed_df.withColumn(
    "time_of_day",
    when((unix_timestamp(col("date"), "yyyy-MM-dd HH:mm:ss").cast("timestamp").substr(12, 2).cast("int") >= 6) &
         (unix_timestamp(col("date"), "yyyy-MM-dd HH:mm:ss").cast("timestamp").substr(12, 2).cast("int") < 12), "Matin")
    .when((unix_timestamp(col("date"), "yyyy-MM-dd HH:mm:ss").cast("timestamp").substr(12, 2).cast("int") >= 12) &
          (unix_timestamp(col("date"), "yyyy-MM-dd HH:mm:ss").cast("timestamp").substr(12, 2).cast("int") < 18), "Après-midi")
    .when((unix_timestamp(col("date"), "yyyy-MM-dd HH:mm:ss").cast("timestamp").substr(12, 2).cast("int") >= 18) &
          (unix_timestamp(col("date"), "yyyy-MM-dd HH:mm:ss").cast("timestamp").substr(12, 2).cast("int") < 24), "Soirée")
    .otherwise("Nuit")
)
...
# ---------------------------------------------------------------------------
# Todo: ajouter toutes les variables calculées
